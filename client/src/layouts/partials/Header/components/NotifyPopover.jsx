...
const NotifyPopover = ({ isLoading, notifications }) => {
  return (
    <Box className="flex flex-col gap-4 w-[350px] relative">
      <Box className="flex justify-between p-4 border-b sticky top-0 right-0 left-0 z-10 !bg-white">
        ...
        {!!notifications?.length && <span className="text-primary cursor-pointer hover:text-primary-dark">
          Đánh dấu là đã đọc
        </span>}
      </Box>
      {isLoading ? (
        ...
        <Box className="flex flex-col p-10 items-center justify-center">
          ...
          <Typography fontSize={"14px"} className="text-neutrals-60">Bạn chưa có thông báo nào</Typography>
        </Box>
      )}
    </Box>
  );
