...
import { updateUser as updateVerify } from "../../services/UserServices";
    ...
    try {
      if (token) {
        ...
        if (res?.data?.status === "success") {
          await updateVerify({ id: user_id, is_verified: true });
          showSuccess("Xác thực thành công");
          dispatch(updateUser({ ...user, is_verified: true }));
  ...
  return (
    <div className="flex flex-col gap-5 pt-10">
      {loading ? (
        ...
        <Container className="p-4 bg-white rounded-md overflow-hidden shadow flex flex-col items-center gap-2">
          <CommonIcon.Verified className="!text-green-700 !text-[100px]" />
          ...
          >
            {is_verified ? "Trang chủ" : "Xác thực ngay"}
          </Button>
        </Container>
      )}
    </div>
